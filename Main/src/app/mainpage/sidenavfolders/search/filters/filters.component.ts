import { Component, ElementRef, EventEmitter, Input, ViewChild, inject } from '@angular/core';
...
import { ObservableInput, debounceTime, of, takeLast } from 'rxjs';
...
import { UserDetailsComponent } from '../people/total/user-details/user-details.component';
import { MatDialog } from '@angular/material/dialog';
import { GuideComponent } from 'src/app/guide/guide.component';
import { GuideForCompanyNameComponent } from 'src/app/guide-for-company-name/guide-for-company-name.component';
...
excludeCompanyDomain:string[]=[]
  ...
  includeCompanyName$!: Observable<string[]>;

  filtersApplied = new EventEmitter<any>();
...
retrieveAndApplyFiltersFromLocalStorage(): void {
  ...
  // Subscribe to filter service updates
  this.filterService.filters$.subscribe((filters) => {
    this.filters = filters;
    this.search();
  });
...
removeJobLevel(level: string): void {
  ...
  applyStoredFilters(storedFilters: any): void {
    ...
    // Emit event to notify filters applied
    this.filtersApplied.emit(true);
    if (storedFilters.include_company_name) {
      this.includeCompanyName = storedFilters.include_company_name.split(', ');
    }
    ...
    if (storedFilters.include_company_domain) {
      this.includeCompanyDomain = storedFilters.include_company_domain.split(', ');
    }
    ...
    if (storedFilters.include_Industry) {
    ...
    if (storedFilters.include_employee_size) {
      this.selectedEmployeeSizes = storedFilters.include_employee_size.split(', ');
    }

    if (storedFilters.include_job_function) {
      this.selectedJobFunctions = storedFilters.include_job_function.split(', ');
    }

    if (storedFilters.include_job_level) {
      this.selectedJobLevels = storedFilters.include_job_level.split(', ');
    }
    this.includeCompanyName$ = of(storedFilters.include_company_name.split(', '));
    // Update the filters in the service
    this.filterService.updateFilters(this.filters);
  }
...
retrieveAndApplySpecificFilters(): void {
  ...
  const storedCountry = localStorage.getItem('country');
  const storedCompanyDomain = localStorage.getItem('companyDomain');
  const storedExcludeCompanyDomain = localStorage.getItem('excludeCompanyDomain');
  const firstName = localStorage.getItem('include_First_Name');
  const storedJobTitle = localStorage.getItem('jobtitle');
  const storedExcludeJobTitle = localStorage.getItem('excludeJobtitle');
  const storedJobLevel = localStorage.getItem('jobLevel');
  ...
  const storedEmployeeSize = localStorage.getItem('empSize');
  const storedCity = localStorage.getItem('city');
  const storedRegion = localStorage.getItem('region');
  const storedIndustry = localStorage.getItem('industry');
  const storedState = localStorage.getItem('state');
  ...
  if (storedCity) this.includeCity = JSON.parse(storedCity);
  if (storedRegion) this.includeRegion = JSON.parse(storedRegion);
  if (storedIndustry) this.includeIndustry = JSON.parse(storedIndustry);
  if (storedState) this.includeState = JSON.parse(storedState);
  if (storedZipcode) this.includeZipcode = JSON.parse(storedZipcode);
  if (storedCompanyName) this.includeCompanyName = JSON.parse(storedCompanyName);
  if (storedExlcudeCompanyName) this.excludeCompanyName = JSON.parse(storedExlcudeCompanyName);
  if (storedCountry) this.includecountry = JSON.parse(storedCountry);
  if (storedCompanyDomain) this.includeCompanyDomain = JSON.parse(storedCompanyDomain);
  if (storedExcludeCompanyDomain) this.excludeCompanyDomain = JSON.parse(storedExcludeCompanyDomain);
  if (firstName) this.filters.include_First_Name = JSON.parse(firstName);
  if (storedJobTitle) this.includeJobTitles = JSON.parse(storedJobTitle);
  if (storedExcludeJobTitle) this.excludeJobTitles = JSON.parse(storedExcludeJobTitle);
  if (storedJobLevel) this.selectedJobLevels = JSON.parse(storedJobLevel);
  if (storedJobFunction) this.selectedJobFunctions = JSON.parse(storedJobFunction);
  if (storedExcludeCountry) this.excludeCountry = JSON.parse(storedExcludeCountry);
  if (savedSelectedEmployeeSizes) {
    this.selectedEmployeeSizes = JSON.parse(savedSelectedEmployeeSizes);
    this.employeeSizeControl.setValue(this.selectedEmployeeSizes);
  }
